...
const PurchaseOrder = ({
  ...
  useEffect(() => {
    if (purchaseEditing && purchaseEditing._id) {
      ...
      // Filter customers based on associated CPOs
    ...
    try {
      ...
      while (hasMore) {
        ...
        customers.forEach((customer) => {
          if (!uniqueCustomerIds.has(customer._id)) {
            uniqueCustomerIds.add(customer._id);
      ...
      setCustomers(allCustomers);
      setFilteredCustomers(allCustomers); // Initialize filtered customers
    } catch (err) {
      console.log(err);
      setCustomers([]);
    }
...
/api/purchases/${purchaseEditing._id}`,
          {
            customer,
            customerpo,
            date,
  ...
  const handleCustomerChange = (e) => {
    ...
    // Update filtered customers based on the selected customer
